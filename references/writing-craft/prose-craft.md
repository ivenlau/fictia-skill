---
name: prose-craft
description: 写作技法 - 身体细节、跑道具、对话权力动态、镜头化写作、标点光谱
...
本文件在 Fictia 工作流中由以下阶段使用：

- **Stage 9（章节写作）**：写作 agent 在生成正文时参考全部 9 个技法节。核心调用点：
  - 身体细节替代情绪词（第 1 节）—— 扫描生成正文，逐个替换情绪词
  - 贯穿道具三次出现（第 2 节）—— 按设定中的物件编排表，在对应小节植入道具
  - 开头事件密度（第 6 节）—— 前 100 字自检，确保 >= 3 个事件
  - 一动一静节奏（第 5 节）—— 小节内交替动/静，防止暴力疲劳或节奏拖沓
  - 三维度揉进（第 8 节）—— 每个详写子事件同时包含发生、感知、反应
  - 标点谱系（第 9 节）—— 对话标点跟随语气和权力关系

- **Stage 10（编辑审查）**：审查 agent 对已完成章节逐项检查：
  - 情绪词残留扫描（第 1 节）
  - 道具出现次数和意义翻转校验（第 2 节）
  - 小节密度诊断（第 7 节）
  - 堆叠式描写检测（第 8 节反模式）
