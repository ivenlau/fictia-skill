...
description: 反AI写作方法论 - 10大AI模式检测器、3遍去AI法、量化评分体系
...
本文件是 Fictia 写作框架的核心参考资料，在以下流水线阶段被引用：

- **Stage 9（正文写作）**：写作 agent 在生成正文时参考本文件的 AI 模式检测规则和核心规则，从源头规避 AI 写作指纹。每次生成段落后，agent 内置自检模块会按模式 1-10 逐项扫描，确保输出文本在生成阶段就避免常见 AI 模式。
- **Stage 10（编辑润色）**：编辑 agent 使用本文件的系统性去AI三遍法对整章进行深度清理。第一遍去泛化，第二遍去书面化，第三遍回自然感。编辑完成后按质量检查清单逐项验收。

本文件在 meta-index 中归类于 `writing-craft` 向量集合，支持语义检索。当 agent 需要查找"如何去掉解释腔"、"情绪外化怎么写"等问题时，可通过向量匹配直接定位到对应章节。
