---
name: genre-writing-formulas
description: 体裁写作公式 - 各类型小说的结构公式和写作模板
...
- **Stage 2（架构）**：根据用户选择的题材/类型，从本文件中匹配对应公式，作为故事结构骨架的输入依据。在架构阶段确定章节结构、情绪节拍曲线、必选场景清单。
- **Stage 8（大纲）**：大纲生成时按公式要求逐章/逐节填充节点、要点，校验情绪节拍完整性、必选场景覆盖率、钩子密度是否达标。
