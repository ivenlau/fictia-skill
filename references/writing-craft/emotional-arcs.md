---
name: emotional-arcs
description: 情绪弧线设计 - 6种弧线类型、情绪递进技法、满足感工程
...
本文档为 Fictia 写作系统中多个阶段提供情绪弧线设计指导：

- **Stage 2（架构）**：选型阶段，根据故事类型和情绪目标选择弧线类型，确定整体情绪走向
- **Stage 4（艺术指导）**：指导场景层面的情绪调度，确保每个场景在"调动-释放"循环中定位清晰
- **Stage 5（叙事编织）**：将弧线转化为具体的叙事节奏，分配字数比例，安排转折点位置
- **Stage 8（大纲设计）**：在大纲层级标注情绪节点、压力升级时机、爽点递增节奏

使用时先查"决策路由表"选定弧线类型，再按对应弧线的字数分配和操作要点展开设计。可叠加两种弧线（如"延迟满足 + 急转"），不超过两种。
