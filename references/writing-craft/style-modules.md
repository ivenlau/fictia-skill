---
name: style-genre-modules
description: 体裁风格模块 - 各类型小说的文风规范和通用流补充
...
本文件供 Fictia 创作流水线以下阶段使用：

- **Stage 3（风格设计）**：根据题材定位匹配对应风格模块，提取核心规则、节奏密度、场景颗粒等参数，生成该书的风格锚定卡。
- **Stage 9（写作）**：正文生成时参照对应题材模块的核心规则与禁止漂移约束，配合「通用正文要求 + 题材正文提示卡 + 本书文风」三件套执行。
