---
name: banned-words
description: 禁用词表 - 两级禁用词、AI味高频词、替代方案
...
> **Fictia 集成说明**：本表供写作流水线两个阶段使用：
> - **Stage 9（写作）**：pre-writing filtering — 在生成正文前加载禁用词表，指导 LLM 主动规避高频 AI 句式和禁用词。
> - **Stage 10（编辑）**：post-writing detection — 对已生成正文进行扫描，命中禁用词/句式时触发自动修正或标注。
>
> 两级禁用词的优先级对应 Gate 机制：Gate A 扫描最毒句式（五星级），Gate B 扫描一级和二级禁用词。
