...
- **整体重构** —— 项目结构、agent 角色、文件组织、流水线的全面调整
- **大段对话** —— 对话已持续多轮、仍涉及多个方向、未收敛到具体任务
- **重大改变** —— 需求涉及多个阶段、多个工作流或核心机制的修改
- **用户提问和质疑** —— 用户连续追问、要求解释决策、要求证明可行性

**默认关闭**，按需启用。**不影响正常阶段执行、局部修改、状态查询、确认类操作**。

### 询问方式

在即将进入受影响的阶段或工作流之前，使用 `AskUserQuestion` 工具询问：

> "本次需求涉及 [具体触发条件描述]。是否开启头脑风暴模式？"
...
用户选择"是"后，**主代理必须为以下每一项创建一个 TaskCreate 任务，并按顺序完成**：

1. **探索项目上下文** —— 检查相关文件、文档、最近的修改记录、对话记录
2. **提出澄清问题** —— 每次只问一个问题（`AskUserQuestion`），了解目的、约束条件、成功标准
3. **提出 2-3 种方案** —— 附带各方案的权衡分析，并给出主代理的明确推荐
4. **展示方案** —— 按复杂度分节呈现，每节展示后获取用户批准
5. **按照批准的方案继续执行** —— 走标准流水线或工作流

### 任务追踪与约束

- 任务创建：用 `TaskCreate` 为每一步创建独立任务；开始时 `TaskUpdate` 标记 `in_progress`，完成时 `completed`
- 步骤 2：一次只问一个问题，避免信息过载
- 步骤 3：必须包含主代理的明确推荐（"我推荐方案 X，因为..."）
- 步骤 4：按方案复杂度从低到高依次展示，便于对比
- 步骤 5：不偏离已批准方案；如有新想法，回到步骤 2/3 重新对齐
